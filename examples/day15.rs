...
fn main() {
    ...
    if false {
        let mut last_turn_for_num = last_turn_for_num.clone();
        let mut last = *input.last().unwrap();
        let start = (input.len() - 1) as u32;
        for last_turn in start..30000000u32 - 1 {
    ...
    {
        // needlessly functional version
        let mut last_turn_for_num = last_turn_for_num.clone();
        let start = (input.len() - 1) as u32;
        const MAX: u32 = 30000000u32 - 1;
        let res2 = (start..MAX).fold(*input.last().unwrap(), |last, last_turn| {
            match last_turn_for_num.entry(last) {
                Occupied(mut x) => {
                    let old_value = *x.get();
                    x.insert(last_turn);
                    last_turn - old_value // last was said in 'old_value' => distance between last_turn and old_value
